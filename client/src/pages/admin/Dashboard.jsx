    ...
    <div className='flex-1 p-4 md:p-10 bg-blue-50/50 min-h-[calc(100vh-70px)]'>
      ...
      <div className='flex flex-wrap gap-4'>
          <div className='flex items-center gap-4 bg-white p-4 min-w-58 rounded shadow cursor-pointer hover:scale-105 transition-all'>
            ...
            <div>
              <p className='text-xl font-semibold text-gray-600'>{dashboardData.books}</p>
              <p className='text-gray-400 font-light'>Books</p>
            </div>
          </div>
            ...
            <table className='w-full text-sm text-gray-500'>
              <thead className='text-xs text-gray-600 text-left uppercase'>
                <tr>
                  ...
                  <th scope='col' className='px-2 py-4 text-left'>Book Title</th>
              ...
              <tbody>
                {dashboardData.recentBooks.map((book,index)=>{
                    return <BookTableItem key={book._id} book={book} fetchBookss={fetchDashboard} index={index + 1}/>
                })}
              </tbody>
